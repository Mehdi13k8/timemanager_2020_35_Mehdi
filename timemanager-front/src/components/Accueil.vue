...
let friday = vendredi.getFullYear() + "-" + ((vendredi.getMonth()+1)<10?'0':'') + (vendredi.getMonth()+1) + "-" + (vendredi.getDate()<10?'0':'') + vendredi.getDate() + " " + "23" + ":" + "59" + ":" + (vendredi.getSeconds()<10?'0':'') + vendredi.getSeconds();
let monday = lundi.getFullYear() + "-" + ((lundi.getMonth()+1)<10?'0':'') + (lundi.getMonth()+1) + "-" + (lundi.getDate()<10?'0':'') + lundi.getDate() + " " + "00" + ":" + "00" + ":" + (lundi.getSeconds()<10?'0':'') + lundi.getSeconds();
alert(monday);
...
export default {
    ...
    getdays: function () {
      ...
      if (this.info) {
        ...
        // console.log(obj[inkey]['start']);
            ...
            // console.log(obj[inkey]['start']);
            const tmp = Math.abs(Date.parse(obj[inkey]['end']) - Date.parse(obj[inkey]['start']));
            const diffTime = tmp / (1000 * 60 * 60); //millisec to heure
            // b.workduration = diffTime; // traduit la valeur
            if (new Date(obj[inkey]['end']).getDay() === new Date(obj[inkey]['start']).getDay()) {
  ...
  mounted() {
    axios
      .get(
        "http://localhost:4000/api/workingtimes/1?start=" + monday /*2019-10-21 08:52:16*/+"&end="+ friday/*2021-10-21 20:52:16*/
      )
      .then(response => {
        this.info = response
      });
